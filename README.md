...
Exit out of the psql REPL and back into your shell
...
Create times table for main and test database
psql -d makersbnb -f db/migrations/03_create_times_table.sql
psql -d makersbnb_test -f db/migrations/03_create_times_table.sql
```
...
Objects: Space
Methods: .add(description) // nice to have: .create_description
...
Objects: Times
Methods: .create(space_id, available_from, available_to)
...
Objects: Times
Methods: .available?
Notes: if the Times.available? == True, then display it on home page, otherwise assume it is taken and not display
```
...
```
Class : Space
Methods : .all - shows all listed spaces
          .add(title, description, price, user_id) - adds a space to the spaces database
```

```
Class : User
Methods : .all - shows all registered users
          .create(first_name, email, password) - signs a new user up to MakersBnB
```

```
Class : Times
Methods : .all - shows all available timeslots for all spaces
          .create(space_id, available_from, available_to) - creates a new available timeslot
          .available?(space_id, request_date) - returns true or false depending on if the specified space is available on the requested date
```
